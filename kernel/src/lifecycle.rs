//! Process lifecycle — creation, exit, wait, fault-kill, and the
//! embedded boot-time programs.
//!
//! This is the "things that change the set of live processes" module.
//! The scheduler (sched.rs) decides *which* live process runs next;
//! this module decides what "live" means.
//!
//! Contents:
//!   - embedded asm programs: init (PID 1), UART server (PID 2), and
//!     the optional security-test process. These are just symbol-
//!     emission blocks; they live here because `launch()` is the only
//!     code that references them.
//!   - `launch()` — boot-time process creation + first sret to U-mode
//!   - `create_process` — raw boot spawn from embedded asm bytes
//!   - `create_process_from_elf` — boot spawn from an ELF in kernel rodata
//!   - `kill_current` — involuntary termination on U-mode fault
//!   - `sys_exit` / `sys_wait` — voluntary termination + reaping
//!
//! `sys_exit` and `kill_current` share a lot of cleanup logic. Folding
//! that into a single `terminate(pid, exit_code) -> NextAction` is a
//! worthwhile follow-up. Not done here to keep this refactor to pure
//! moves.
...
use crate::process::{PROCS, CURRENT_PID, IRQ_OWNER, Process, ProcessState, NetOp};
use crate::security::{self, MAX_PROCS};
use crate::trap::TrapFrame;
use crate::{println, kdump_csrs, kdump_plic, kdump_procs};

// ── Embedded User Programs ─────────────────────────────────────
//
// These `global_asm!` blocks emit `.text` symbols that the linker
// binds into the kernel image. `launch()` copies them into userspace
// pages at boot. They're here, not in process.rs, because the only
// things that reference them are `launch()` and `create_process()`.
...
///
/// Called by the trap handler when a U-mode exception occurs (page
/// fault, illegal instruction, etc.). This is the microkernel
/// equivalent of Linux's SIGSEGV + process termination.
///
/// `exit_code` follows Unix convention: 128 + signal number (here,
/// scause code).
pub fn kill_current(frame: &mut TrapFrame, exit_code: usize) {
    // SAFETY: INV-1. Single-hart kernel; called from trap context.
    ...
    // SAFETY: INV-1. All mutations of PROCS are under the single-hart
    // invariant; the scheduler swap at the end is INV-2 (frame
    // exclusivity).
    unsafe {
...
pub fn launch() -> ! {
    ...
    //
    // SAFETY: INV-7. CSR manipulation in S-mode.
    ...
    #[cfg(feature = "security-test")]
    {
        ...
        // SAFETY: INV-4. Linker-defined symbols have valid addresses.
    ...
    #[cfg(not(any(feature = "security-test", feature = "rust-user")))]
    {
        ...
        // SAFETY: INV-4. Linker-defined symbols.
        ...
        // SAFETY: INV-1. PID 2 was just created; no other code touches it.
    ...
    // SAFETY: INV-8. page_alloc::init has run (boot::stage_memory).
    ...
    // SAFETY: INV-1. We just created PID 1; nothing else touches it.
    ...
    // SAFETY: INV-7. Privileged CSR manipulation + sret from S-mode.
    unsafe {
        asm!(
            "csrw sepc, {entry}",
            "csrr t0, sstatus",
...
fn create_process(
    ...
    // SAFETY: INV-5. alloc_zeroed_page returned a kernel-writable PA;
    // code_start is a linker symbol address (INV-4).
    unsafe {
...
///
/// Like sys_spawn but reads ELF data from kernel .rodata (via
/// include_bytes!) instead of user memory. Used at boot to load
/// compiled Rust userspace binaries.
    ...
    for seg_idx in 0..info.num_segments {
        ...
        for p in 0..num_pages {
            ...
            if copy_start < copy_end {
                ...
                // SAFETY: INV-5 for dst, INV-9 for src (length-checked above).
                unsafe {
...
pub fn sys_exit(frame: &mut TrapFrame) {
    ...
    // SAFETY: INV-1 + INV-2 for the frame swap.
    unsafe {
        PROCS[current].state = ProcessState::Free;
        PROCS[current].exit_code = exit_code;
...
/// with the stored exit code. Otherwise blocks until the child calls
/// SYS_EXIT.
pub fn sys_wait(frame: &mut TrapFrame) {
    ...
    // SAFETY: INV-1 + INV-2 for schedule() call at the end.
    unsafe {
